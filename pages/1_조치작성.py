...
available_branches = [b for b in BRANCH_ORDER if b in df["관리지사표시"].unique()]
...
owners = sorted(df_f["담당자"].dropna().unique().tolist()) if "담당자" in df_f else []
selected_owner = st.sidebar.radio("이름(담당자)", ["전체"] + owners)
